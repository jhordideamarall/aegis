...
export default function InstagramIntegrationPage() {
  return (
    <div className="p-4 md:p-6 max-w-5xl mx-auto space-y-10 pb-20">
      ...
      {/* Hero Section */}
      <div className="relative overflow-hidden rounded-[2.5rem] bg-slate-900 p-8 md:p-16 text-center space-y-6 shadow-2xl shadow-slate-200">
        ...
        <div className="flex items-center justify-center gap-2 pt-4">
          <div className="flex -space-x-2">
            {[1,2,3,4].map(i => <div key={i} className="w-8 h-8 rounded-full border-2 border-slate-900 bg-slate-800 flex items-center justify-center text-[10px] font-black text-slate-500">U{i}</div>)}
          </div>
          <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-2">Joining 500+ businesses</span>
        </div>
      </div>
      ...
      <Card className="border-none bg-slate-50 rounded-3xl overflow-hidden">
        <CardContent className="p-8 flex flex-col md:flex-row items-center justify-between gap-6">
           ...
           <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] font-black text-slate-300 uppercase tracking-widest italic">Syncing with Meta API...</span>
           </div>
        </CardContent>
      </Card>
    </div>
